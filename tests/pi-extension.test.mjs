...
test('Pi extension maps decisions, handles noninteractive asks, and records human results', async () => {
  ...
  try {
    ...
    const source = (await readFile(new URL('../extensions/pi.ts', import.meta.url), 'utf8')).replace('const executable = "agy-auto-approve";', `const executable = ${JSON.stringify(executable)};`);
